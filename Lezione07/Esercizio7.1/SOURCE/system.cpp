...
void System :: initialize_properties(){ // Initialize data members used for measurement of properties
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      string file_pot_en;
      if(_sim_type == 0){
        if(_restart) file_pot_en = "../OUTPUT/actual_pe_MD.dat";
        else         file_pot_en = "../OUTPUT/equi_pe_MD.dat";
      } else if(_sim_type == 1){
        if(_restart) file_pot_en = "../OUTPUT/actual_pe_MC.dat";
        else         file_pot_en = "../OUTPUT/equi_pe_MC.dat";
      } else {
        file_pot_en = "../OUTPUT/potential_energy.dat";
      }

        ofstream coutp(file_pot_en);
        coutp << "#ACTUAL_PE:" << endl;
        coutp.close();
        _nprop++;
        ...
        index_property++;
          ...
          string file_temp;
          if(_restart==0 and _sim_type==0){
            file_temp = "../OUTPUT/equi_temp_MD.dat";
          } else if (_restart==0 and _sim_type==1){
            file_temp = "../OUTPUT/equi_temp_MC.dat";
          } else {
            file_temp="../OUTPUT/temperature.dat";
          }
        ofstream coutte(file_temp);
        coutte << "#ACTUAL_T:" << endl;
        coutte.close();
        _nprop++;
        _measure_temp = true;
