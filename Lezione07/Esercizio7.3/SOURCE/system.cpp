  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        string file_pot_en;
        if(_sim_type==0){
          file_pot_en = "../OUTPUT/potential_energy_MD.dat";
        } else if(_sim_type==1){
          file_pot_en = "../OUTPUT/potential_energy_MC.dat";
        } else {
          file_pot_en="../OUTPUT/potential_energy.dat";
        }

        ofstream coutp(file_pot_en);
        ...
        index_property++;
        _vtail = 8.0 * M_PI * _rho * (1.0/(9.0*pow(_r_cut,9))-1.0/(3.0*pow(_r_cut,3)));
        ...
        string file_temp;
        if(_sim_type==0){
          file_temp = "../OUTPUT/temperature_MD.dat";
        } else if(_sim_type==1){
          file_temp = "../OUTPUT/temperature_MC.dat";
        } else{
          file_temp = "../OUTPUT/temperature.dat";
        }

        ofstream coutte(file_temp);
        ...
        string file_press;
        if(_sim_type==0){
          file_press = "../OUTPUT/pressure_MD.dat";
        } else if(_sim_type==1){
          file_press = "../OUTPUT/pressure_MC.dat";
        }
        else{
          file_press = "../OUTPUT/pressure.dat";
        }

        ofstream coutpr(file_press);
        ...
        string file_gofr;
        if(_sim_type==0){
          file_gofr = "../OUTPUT/gofr_MD.dat";
        } else if(_sim_type==1){
          file_gofr= "../OUTPUT/gofr_MC.dat";
        } else {
          file_gofr="../OUTPUT/gofr.dat";
        }

        ofstream coutgr(file_gofr);
        ...
        input>>_n_bins_v;
        _nprop += _n_bins_v;
        _bin_size_v = 3.0*sqrt(3.0*_temp)/((double)_n_bins_v);
        _measure_pofv = true;
...
void System :: measure(){ // Measure properties
    ...
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){
        ...
        if(_measure_gofr){
          ...
          if(bin < _n_bins){
          _measurement(_index_gofr + bin) += 2;  // coppia conta due volte (i≠j)
        }
  ...
  if (_measure_pressure){
    if(_sim_type == 1){ // per MC devo usare _temp che è fissata
...
void System :: averages(int blk){
  ...
  if (_measure_penergy){
    ...
    coutf.open(file_pot_en,ios::app);
...
if (_measure_gofr){
  ...
  coutf.open(file_gofr);
  for(int i=0; i<_n_bins; i++){
    
    double r_low   = i * _bin_size;
    double r_high  = (i + 1) * _bin_size;
    double r_center = (i + 0.5) * _bin_size;

    // Volume della shell sferica
    double volume = (4.0/3.0) * M_PI * (pow(r_high, 3) - pow(r_low, 3));

    // Normalizzazione
    double norm = volume * _rho * double(_npart);

    average     = _average(_index_gofr + i) / norm;
    sum_average = _global_av(_index_gofr + i) / (norm);
    sum_ave2    = _global_av2(_index_gofr + i) / (norm * norm);

    double error = this->error(sum_average, sum_ave2, blk);

    coutf << setw(12) << r_center
          << setw(12) << average
          << setw(12) << sum_average / double(blk)
          << setw(12) << error << endl;
  }
