...
void System :: initialize(){ // Initialize the System object according to the content of the input files in the ../INPUT/ directory
  ...
  while ( !input.eof() ){
    ...
    if( property == "SIMULATION_TYPE" ){
      ...
      input >> _time_reverse;
      if(_time_reverse) coutf << "TIME REVERSAL ENABLED" << endl;
  ...
  // se c'è attivo il time_reverse inverte le velocità
  if(_sim_type==0 && _time_reverse) this->reverse_velocities();
...
void System :: initialize_velocities(){
  ...
  if(_restart){
    // RESTART=1: legge conf-1.xyz come xold (posizione al passo precedente)
    // e config.xyz è già stato letto come xnew da read_configuration()
    ifstream cinf;
    cinf.open("../INPUT/CONFIG/conf-1.xyz");
    if(cinf.is_open()){
      string comment, particle;
      int ncoord;
      cinf >> ncoord;
      if(ncoord != _npart){
        cerr << "PROBLEM: conflicting number of coordinates in conf-1.xyz!" << endl;
        exit(EXIT_FAILURE);
      }
      cinf >> comment;
      for(int i=0; i<_npart; i++){
        cinf >> particle >> xold >> yold >> zold;
        // conf-1.xyz ha coordinate normalizzate per _side, come config.xyz
    ...
    cinf.close();
    // Calcola le velocità da xnew e xold appena letti
    // v = (xnew - xold) / (2*delta)
    for(int i=0; i<_npart; i++){
      _particle(i).setvelocity(0, this->pbc(_particle(i).getposition(0,true) - _particle(i).getposition(0,false), 0) / (2.0*_delta));
      _particle(i).setvelocity(1, this->pbc(_particle(i).getposition(1,true) - _particle(i).getposition(1,false), 1) / (2.0*_delta));
      _particle(i).setvelocity(2, this->pbc(_particle(i).getposition(2,true) - _particle(i).getposition(2,false), 2) / (2.0*_delta));
    }
  } else {
    // RESTART=0: inizializzazione da zero con velocità casuali
    vec vx(_npart), vy(_npart), vz(_npart);
    vec sumv(_ndim);
    sumv.zeros();
    for(int i=0; i<_npart; i++){
      int appo = floor(_rnd.Rannyu(0,6));
      double vT = sqrt(3.0*_temp);
      if(appo==0){vx(i)= vT; vy(i)=0.0; vz(i)=0.0;}
      if(appo==1){vx(i)=-vT; vy(i)=0.0; vz(i)=0.0;}
      if(appo==2){vx(i)=0.0; vy(i)= vT; vz(i)=0.0;}
      if(appo==3){vx(i)=0.0; vy(i)=-vT; vz(i)=0.0;}
      if(appo==4){vx(i)=0.0; vy(i)=0.0; vz(i)= vT;}
      if(appo==5){vx(i)=0.0; vy(i)=0.0; vz(i)=-vT;}
      sumv(0) += vx(i);
      sumv(1) += vy(i);
      sumv(2) += vz(i);
    }
    // Rimuovi il momento totale (centro di massa fermo)
    ...
    sumv2 /= double(_npart);
    scalef = sqrt(3.0*_temp / sumv2); // riscala per ottenere la T desiderata
    for(int i=0; i<_npart; i++){
      _particle(i).setvelocity(0, vx(i)*scalef);
      _particle(i).setvelocity(1, vy(i)*scalef);
      _particle(i).setvelocity(2, vz(i)*scalef);
    }
    // Calcola xold = xnew - v*delta (un passo indietro nel tempo)
    for(int i=0; i<_npart; i++){
...
void System :: reverse_velocities(){
  // TIME REVERSAL: inverte la direzione del tempo nello schema di Verlet scambiando xold <-> xnew per ogni particella.
  // Poiche' xnew = 2*xcur - xold + F*dt^2, dopo lo swap il Verlet integra nella direzione temporale opposta.
  double xnew, ynew, znew, xold, yold, zold;
  for(int i=0; i<_npart; i++){
    // Salva le posizioni correnti
    xnew = _particle(i).getposition(0, true);   // posizione attuale (xnew)
    ynew = _particle(i).getposition(1, true);
    znew = _particle(i).getposition(2, true);
    
    xold = _particle(i).getposition(0, false);  // posizione precedente (xold)
    yold = _particle(i).getposition(1, false);
    zold = _particle(i).getposition(2, false);

    // Swap: xnew diventa xold e viceversa -> il tempo scorre al contrario
    _particle(i).setposition(0, xold);
    _particle(i).setposition(1, yold);
    _particle(i).setposition(2, zold);

    _particle(i).setpositold(0, xnew);
    _particle(i).setpositold(1, ynew);
    _particle(i).setpositold(2, znew);
    
    // Inverte anche le velocità
  ...
  coutf << "TIME REVERSAL APPLIED: xold <-> xnew swapped, velocities inverted." << endl;
  coutf.close();
  return;
}
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        if(_time_reverse==0 && _nblocks==250){
          coutp.open("../OUTPUT/potential_energy_for250.dat");
        } else if(_time_reverse==0 && _nblocks==25){
          coutp.open("../OUTPUT/potential_energy_for25.dat");
        } else if(_time_reverse && _nblocks==250){
          coutp.open("../OUTPUT/potential_energy_rev250.dat");
        } else if(_time_reverse && _nblocks==25){
          coutp.open("../OUTPUT/potential_energy_rev25.dat");
        } else {
          coutp.open("../OUTPUT/temperature.dat");
        }
        ...
        if(_time_reverse==0 && _nblocks==250){
          coutte.open("../OUTPUT/temperature_for250.dat");
        } else if(_time_reverse==0 && _nblocks==25){
          coutte.open("../OUTPUT/temperature_for25.dat");
        } else if(_time_reverse && _nblocks==250){
          coutte.open("../OUTPUT/temperature_rev250.dat");
        } else if(_time_reverse && _nblocks==25){
          coutte.open("../OUTPUT/temperature_rev25.dat");
        } else {
          coutte.open("../OUTPUT/temperature.dat");
        }
        ...
        input>>_n_bins_v;
        _nprop += _n_bins_v;
        _bin_size_v = 3.0*sqrt(3.0*_temp)/((double)_n_bins_v); // TO BE FIXED IN EXERCISE 4
...
void System :: measure(){ // Measure properties
  ...
  if(_measure_pofv){
    double vx; double vy; double vz;
    for(int i=0; i<_npart; i++){
      vx=_particle(i).getvelocity(0);
      vy=_particle(i).getvelocity(1);
      vz=_particle(i).getvelocity(2);
...
void System :: averages(int blk){
  ...
  if (_measure_penergy){
    ...
    if(_time_reverse==0 && _nblocks==250){
      coutf.open("../OUTPUT/potential_energy_for250.dat",ios::app);
    } else if(_time_reverse==0 && _nblocks==25){
      coutf.open("../OUTPUT/potential_energy_for25.dat",ios::app);
    } else if(_time_reverse && _nblocks==250){
      coutf.open("../OUTPUT/potential_energy_rev250.dat",ios::app);
    } else if(_time_reverse && _nblocks==25){
      coutf.open("../OUTPUT/potential_energy_rev25.dat",ios::app);
    } else {
  ...
  if (_measure_temp){
    ...
    if(_time_reverse==0 && _nblocks==250){
      coutf.open("../OUTPUT/temperature_for250.dat",ios::app);
    } else if(_time_reverse==0 && _nblocks==25){
      coutf.open("../OUTPUT/temperature_for25.dat",ios::app);
    } else if(_time_reverse && _nblocks==250){
      coutf.open("../OUTPUT/temperature_rev250.dat",ios::app);
    } else if(_time_reverse && _nblocks==25){
      coutf.open("../OUTPUT/temperature_rev25.dat",ios::app);
    } else {
  ...
  if (_measure_pofv){
    ...
    for(int i = 0; i < _n_bins_v; i++){
        ...
        average     = _average(_index_pofv + i) / norm;
        sum_average = _global_av(_index_pofv + i) / norm;
        sum_ave2    = _global_av2(_index_pofv + i) / (norm * norm);

        double errore = this->error(sum_average, sum_ave2, blk);

        coutf << setw(12) << v_center
              << setw(12) << average                    // media del blocco
