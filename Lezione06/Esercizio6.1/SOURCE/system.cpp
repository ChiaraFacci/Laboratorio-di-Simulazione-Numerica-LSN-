...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){ //Gibbs sampler for Ising     // TO BE FIXED IN EXERCISE 6

    double H_eff=_J * (_particle(this->pbc(i-1)).getspin() + _particle(this->pbc(i+1)).getspin() ) + _H; //campo efficace
    double prob= 1.0 / (1.0 + exp (-2.0 * _beta * H_eff)); //prob slide 42
...
void System :: initialize(){ // Initialize the System object according to the content of the input files in the ../INPUT/ directory
  ...
  Primes >> p1 >> p2 ;
  Primes >> p1 >> p2 ; // AGGIUNTO IO PER POFV
  Primes.close();
...
void System :: measure(){ // Measure properties
  ...
  if(_measure_pofv){
    ...
    double vx; double vy; double vz; double v;
    for(int k=0; k<_npart; k++){
      vx = _particle(k).getvelocity(0);
      vy = _particle(k).getvelocity(1);
      vz = _particle(k).getvelocity(2);

      v = sqrt(pow(vx,2)+pow(vy,2)+pow(vz,2));
      bin = int(v / _bin_size_v);
  ...
  if(_measure_cv){
    //Cv/N = beta^2 * ( <U^2> - <U>^2 )
      ...
      for (int i=0; i<_npart; i++){
        s_i = double(_particle(i).getspin());
        s_j = double(_particle(this->pbc(i+1)).getspin());
        u2 += - _J * s_i * s_j - 0.5 * _H * (s_i + s_j);

        _measurement(_index_cv) = u2 * u2 / double(_npart*_npart);
        // non sto accumulando cv perchè la formula richiede sia <U^2> che <U>^2 mediati su blocchi diversi
        // qua sto registrando proprio <U^2>

        // il calcolo corretto di cv verrà fatto in averages() dove sono disponibili tutte medie a blocchi
      }
  }
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
// TO BE FIXED IN EXERCISE 6
  if(_measure_chi){
    // chi = beta * (sum s_i)^2 / N
    ...
    double susceptibility = _beta * m_temp * m_temp / _npart;

    _measurement(_index_chi) = susceptibility;
  }
...
void System :: averages(int blk){
  ...
  if (_measure_tenergy){
   ...
   if(blk==_nblocks and _sim_type==2 and _H==0){
      ofstream coutf2;
      coutf2.open("../OUTPUT/tenergy_temp_metro.dat",ios::app);
      coutf2 << setw(12) << _temp
      << setw(12) << sum_average/double(blk)
      << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
      coutf2.close();
    }

      if(blk==_nblocks and _sim_type==3 and _H==0){
      ofstream coutf2;
      coutf2.open("../OUTPUT/tenergy_temp_gibbs.dat",ios::app);
      coutf2 << setw(12) << _temp
      << setw(12) << sum_average/double(blk)
      << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
      coutf2.close();
 ...
 if (_measure_magnet){
   ...
   if(blk==_nblocks and _sim_type==2 and _H>0){
      ofstream coutf2;
      coutf2.open("../OUTPUT/magn_temp_metro.dat",ios::app);
      coutf2 << setw(12) << _temp
      << setw(12) << sum_average/double(blk)
      << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
      coutf2.close();
      ...
      if(blk==_nblocks and _sim_type==3 and _H>0){
      ofstream coutf2;
      coutf2.open("../OUTPUT/magn_temp_gibbs.dat",ios::app);
      coutf2 << setw(12) << _temp
      << setw(12) << sum_average/double(blk)
      << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
      coutf2.close();
  ...
  // _average(_index_cv)      -> <U^2> medio sul blocco corrente (accumulato in measure())
  // _average(_index_tenergy) -> <U> medio sul blocco corrente

    double cv_block = _beta * _beta  * _npart * ( _average(_index_cv) - pow(_average(_index_tenergy), 2) );

    // Correzione degli accumulatori globali:
    // _global_av e _global_av2 erano stati aggiornati in averages() con i valori grezzi di <U^2> (non di Cv). 
    // Li sostituiamo ora con cv_block per garantire che la media progressiva e l'errore statistico siano calcolati correttamente su Cv e non su U^2.
    _global_av(_index_cv)  += cv_block - _average(_index_cv);
    _global_av2(_index_cv) += cv_block*cv_block - _average(_index_cv)*_average(_index_cv);
    _average(_index_cv) = cv_block;
   ...
   if(blk==_nblocks and _sim_type==2 and _H==0){
      ofstream coutf2;
      coutf2.open("../OUTPUT/cv_temp_metro.dat",ios::app);
      coutf2 << setw(12) << _temp
      << setw(12) << sum_average/double(blk)
      << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
      coutf2.close();
      ...
      if(blk==_nblocks and _sim_type==3 and _H==0){
      ofstream coutf2;
      coutf2.open("../OUTPUT/cv_temp_gibbs.dat",ios::app);
      coutf2 << setw(12) << _temp
      << setw(12) << sum_average/double(blk)
      << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
      coutf2.close();
  ...
  // TO BE FIXED IN EXERCISE 6
   ...
   if(blk==_nblocks and _sim_type==2 and _H==0){
      ofstream coutf2;
      coutf2.open("../OUTPUT/chi_temp_metro.dat",ios::app);
      coutf2 << setw(12) << _temp
      << setw(12) << sum_average/double(blk)
      << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
      coutf2.close();
      ...
      if(blk==_nblocks and _sim_type==3 and _H==0){
      ofstream coutf2;
      coutf2.open("../OUTPUT/chi_temp_gibbs.dat",ios::app);
      coutf2 << setw(12) << _temp
      << setw(12) << sum_average/double(blk)
      << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
      coutf2.close();
